...
class Uninstall(Command):
    ...
    def run(self):
        ...
        for file in reversed(sorted(files)):
            ...
            if dirname not in dirs and os.path.isdir(dirname) and len(os.listdir(dirname)) == 0:
                dirs.add(dirname)
                ...
                if dirname.find("site-packages/") > 0:
                    info("removing %s" % repr(dirname))
                    if not self.dry_run:
                        try:
                            os.rmdir(dirname)
                        except OSError as e:
                    ...
                    info("skipping empty directory %s" % repr(dirname))
...
class InstallData(install_data):
    ...
    def _update_icon_cache(self):
        ...
        try:
            subprocess.call(
                ["gtk-update-icon-cache", "-q", "-f", "-t", os.path.join(self.install_dir, "share/icons/hicolor")])
        except Exception as e:
    ...
    def _find_css_files(self):
        ...
        for css_dir in glob.glob(os.path.join(CSS_DIR, '*')):
            src = glob.glob(os.path.join(css_dir, 'gtk-3.0', 'apps', '*.css'))
